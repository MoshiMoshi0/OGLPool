...
int ContactManifold::areaSort( ManifoldPoint* pt ){
	...
	if (maxPenetrationIndex != 0) {
		vec3 a0 = pt->point0 - points[1]->point0;
		vec3 b0 = points[3]->point0 - points[2]->point0;
		vec3 cr = cross(a0, b0);
		res0 = dot( cr, cr );
	}

	if (maxPenetrationIndex != 1) {
		vec3 a1 = pt->point0 - points[0]->point0;
		vec3 b1 = points[3]->point0 - points[2]->point0;
		vec3 cr = cross(a1, b1);
		res1 = dot( cr, cr );
	}

	if (maxPenetrationIndex != 2) {
		vec3 a2 = pt->point0 - points[0]->point0;
		vec3 b2 = points[3]->point0 - points[1]->point0;
		vec3 cr = cross(a2, b2);
		res2 = dot( cr, cr );
	}

	if (maxPenetrationIndex != 3) {
		vec3 a3 = pt->point0 - points[0]->point0;
		vec3 b3 = points[2]->point0 - points[1]->point0;
		vec3 cr = cross(a3, b3);
		res3 = dot( cr, cr );
	}
...
int ContactManifold::getCacheEntry( ManifoldPoint* pt ){
	float shortestDist = 0.02 * 5;// getContactBreakingThreshold() * getContactBreakingThreshold();

	int nearestPoint = -1;
	for( int i = 0; i < pointsAdded; i++ ) {
		ManifoldPoint* mp = points[i];

		vec3 diffA = mp->point0 - pt->point0;
...
void ContactManifold::setBodies( RigidBody* e0, RigidBody* e1 ){
